...
ADC_COUNTER  DS.W  1                 ; Counter for ADC samples (0-2047)
ADC_RESULT   DS.B  1                 ; Most recent ADC conversion result
ADC_FINISHED DS.B  1                 ; Flag indicating ADC has completed (1=finished)
ADC_MSG      DC.B  'analog signal acquisition (8kHz, 2048 samples) ....', $00  ; Message for ADC command
ADC_FINISH_MSG DC.B 'Finished', $00  ; Message for when ADC completes
...
mainLoop    
            ...
            ; Critical section - process data safely
            sei                      ; Disable interrupts
            
            ; Data is ready, get the value and clear flag immediately
            ldab    waveDataVal      ; Get the value into B
            clr     waveDataReady    ; Clear the flag ASAP
            
            ; End critical section
            cli                      ; Re-enable interrupts
            
            ; Now safely process the data outside critical section
            clra                     ; Clear A for 16-bit number
            jsr     pnum10           ; Print the number (with newline)
            
            ; Fall through to check if wave finished
            
checkWaveComplete
            ; Quick check for user input before checking wave complete
            ; This improves responsiveness during high-speed sampling
            jsr     getchar
            tsta
            bne     handleUserInput  ; If input received, handle it
            ...
            ; Check if ADC sampling has completed
            ldaa    ADC_FINISHED
            beq     skipFinishMsg    ; If not ADC completion, skip "Finished" message
            
            ; Print "Finished" message for ADC completion
...
handleUserInput:   ; New label for handling user input
...
executeADCCommand:
            ...
            ; Ensure ADC is properly initialized
            sei                     ; Critical section - disable interrupts
            
            ; Double-check ADC configuration registers
            ldaa  #%11000000      ; Turn ON ADC, clear flags, Disable ATD interrupt
            staa  ATDCTL2
            ldaa  #%00001000      ; Single conversion per sequence, no FIFO
            staa  ATDCTL3
            ...
            ; Reset ADC sample counter and flags
            ldd    #0
            std    ADC_COUNTER
            clr    ADC_FINISHED    ; Clear the ADC finished flag
            clr    waveDataReady   ; Ensure data ready flag is clear to start
            clr    waveCompleteFlag ; Ensure complete flag is clear
            ...
            ; Start timer and ADC sampling
            jsr    StartTimer5oc   ; Setup timer and start first ADC conversion
...
StartTimer5oc                 ; <<< RENAMED from StartTimer6oc
            ...
            ; Disable timer interrupt while configuring
            LDAA   #0
            STAA   TIE              ; Temporarily disable all timer interrupts
            
            ; Configure Timer Channel 5 for Output Compare
            ...
            ; Calculate first interrupt time
            LDD    TCNTH             ; Get current timer counter
            ADDD   #3000             ; 125usec with (24MHz/1 clock)
            STD    TC5H              ; <<< CHANGED to TC5H

            ; Now enable Timer Channel 5 interrupt
            LDAA   #%00100000        ; <<< CHANGED to bit 5 for channel 5
            STAA   TIE               ; set CH5 interrupt Enable
            
            ; Start ADC conversion for first sample
            LDAA   #%10000111        ; Right justified, unsigned, single conversion, channel 7
            STAA   ATDCTL5           ; Start first ADC conversion
...
oc5_handleADC:
            ...
            ; Ensure we're not setting the flag if we're still processing previous data
            ; This prevents buffer overrun in the output routine
            ldaa  waveDataReady
            cmpa  #1                   ; Is data still waiting to be processed?
            beq   skipSetReadyFlag     ; If yes, skip setting the ready flag this time
            
            ; Set flag to print in main loop
            ldaa  #1                   
            staa  waveDataReady        
